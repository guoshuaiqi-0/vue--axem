<template>
	<div class="JSquestionBank">
		...
		<div class="item mr-20" v-for="(item,index) in listRows" :key="index">
			<div class="header">
				...
				<div class="picture_button">
					<img src="../assets/icon_chazhao.png" class="icon" alt="" @click="createModify(item)">
					<img src="../assets/icon_lajitong.png" class="icon mar-10" alt=""
			...
			<div class="main">
				...
				<div class="radio mar-20">
					<el-radio-group v-model="radio.id">
						<el-radio label="A" disabled class="mar-50" v-if="item.questionA">A. {{item.questionA}}
						</el-radio>
						<el-radio label="B" disabled class="mar-50" v-if="item.questionB">B. {{item.questionB}}
						</el-radio>
						<el-radio label="C" disabled class="mar-50" v-if="item.questionC">C. {{item.questionC}}
						</el-radio>
						<el-radio label="D" disabled class="mar-50" v-if="item.questionD">D. {{item.questionD}}
						</el-radio>
					</el-radio-group>
				</div>
			...
			<div class="footer">
				<p class="mar-30">所属知识点：<span class="mar-50">知识点一</span><span class="mar-50">知识点一</span></p>
				<p class="mar-30">解析：这是解析字段</p>
			</div>
		</div>
		<div class="block">
			<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
				:page-sizes="[5,10,15,20,30,50]" :page-size="listParams.pageSize"
		...
		<!-- 创建问题 -->
		<el-drawer title="我是标题" :visible.sync="createQuestion" :with-header="false" size="50%" class="creat_question"
			...
			<div class="box	">
				<div class="form_Question">
					...
					<el-row class="title align mr-20">
						...
						<el-col :span="16">
							<el-input v-model="creatParams.title" placeholder="请输入内容"></el-input>
						</el-col>
					</el-row>
					<el-row class="align mr-20" v-for=" item in questionOption" :key="item.id">
						<el-col :span="4">{{ item.label }}：</el-col>
						<el-col :span="16">
							<el-input v-model="item.value" placeholder="请输入内容"></el-input>
						</el-col>
						<el-col :span="2" v-if="item.id == questionOption.length  && item.id <4">
							<i class="el-icon-circle-plus-outline i" @click="addQuestionOption()"></i>
						</el-col>
						<el-col :span="1" v-if="item.id == questionOption.length  && item.id > 2">
							<i class="el-icon-remove-outline i" @click="removeQuestionOption(item.id)"></i>
						</el-col>
					</el-row>
					<el-row class="answer align mr-20">
						<el-col :span="4">答案：</el-col>
						<el-col :span="16">
							<el-select v-model="creatParams.answer" placeholder="请选择" class="select">
								<el-option v-for="(item,index) in answer" :key="index" :label="item.label"
									:value="item.value">
								</el-option>
							</el-select>
						</el-col>
					...
					<el-row class="level align mr-20">
						...
						<el-col :span="16">
							<el-select v-model="creatParams.level" placeholder="请选择" class="select">
					...
					<el-row class="submite align mr-20">
						<el-col :span="6" :offset="4">
							<el-button type="primary" class="select" @click="createModifySubmite">提交</el-button>
						</el-col>
						<el-col :span="6" :offset="4">
							<el-button type="info" class="select" @click="close()">取消</el-button>
						</el-col>
					</el-row>
				</div>
			</div>
		...
		methods: {
			...
			checkParams(obj = {}) {
				...
				for (var key in obj) {
					...
					if (key == 'questionC' && this.questionOption.length == 3 && obj[key] == '') {
						this.$message({
							type: 'info',
							message: `选项C不能为空`
						});
						debuge = false
						return
					} else if (key == 'questionD' && this.questionOption.length == 4 && obj[key] == '') {
						this.$message({
							type: 'info',
							message: `选项D不能为空`
						});
